...
void ana_one(int threshold, int max_gap, int range, float min_length , int muon_length, int plane, int nentry){
   ...
   TFile *infile = new TFile("/exp/sbnd/data/users/brindenc/PAD/intime/hitdumper_small.root");
   ...
   std::cout << "coords0 size:" << coords0.size() << std::endl;
   std::cout << "coords1 size:" << coords1.size() << std::endl;
   ...
   std::cout << "number of tpc 0 lines: " << lines0.size() << std::endl;
   std::cout << "number of tpc 1 lines: " << lines1.size() << std::endl;
   ...
   //Set title to entry number
   c1->SetTitle(Form("Event %d",nentry));
   //Write canvas to .png
   c1->SaveAs("hough_ana.png");
